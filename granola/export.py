...
def build_output_path(artifacts_dir: Path, created_at: str, prefix: str, title: str | None, note_id: str, suffix: str) -> Path:
...
def export_note_files(note: dict, artifacts_dir: Path, *, include_json: bool, include_summary: bool, include_transcript: bool) -> list[Path]:
    ...
    if include_json:
        path = build_output_path(artifacts_dir, created_at, "note", title, note_id, "json")
        path.write_text(raw_json_text(note), encoding="utf-8")
        written.append(path)

    if include_summary:
        path = build_output_path(artifacts_dir, created_at, "summary", title, note_id, "txt")
        path.write_text(summary_text(note), encoding="utf-8")
        written.append(path)

    if include_transcript:
        path = build_output_path(artifacts_dir, created_at, "transcript", title, note_id, "txt")
