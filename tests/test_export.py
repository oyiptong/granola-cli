...
from granola.export import build_output_path, export_note_files, raw_json_text, summary_text, transcript_text
...
def test_filename_generation() -> None:
    note = sample_note()
    path = build_output_path(tmp_artifacts_dir(), note["created_at"], "summary", note["title"], note["id"], "txt")
    assert path.name == "2026-01-27_summary_quarterly-yoghurt-budget-review_not_1d3tmYTlCICgjy.txt"


def test_export_note_files(tmp_path) -> None:
    note = sample_note()
    written = export_note_files(note, tmp_path, include_json=True, include_summary=True, include_transcript=True)
